...
-- Helper to safely get effects by type, preferring modern CoreRPG or 5E-specific EffectManager if available.
local function getEffectsByTypeSafe(nodeActor, sEffectType, aFilter, rTarget, bTargetedOnly)
    if EffectManager.getEffectsByType then
        return EffectManager.getEffectsByType(nodeActor, sEffectType, rTarget, bTargetedOnly)
    end
    if EffectManager5E and EffectManager5E.getEffectsByType then
        return EffectManager5E.getEffectsByType(nodeActor, sEffectType, aFilter, rTarget, bTargetedOnly)
    end
    return {}
end

-- Helper to safely get effects bonus, preferring EffectManager5E if available.
local function getEffectsBonusSafe(nodeActor, aEffectTypes, bFirstOnly, aFilter)
    if EffectManager5E and EffectManager5E.getEffectsBonus then
        return EffectManager5E.getEffectsBonus(nodeActor, aEffectTypes, bFirstOnly, aFilter)
    end
    return {}, 0, 0
end

-- Helper to safely get ability effects bonus, preferring ActorManager5E if available.
local function getAbilityEffectsBonusSafe(nodeActor, sAbility)
    if ActorManager5E and ActorManager5E.getAbilityEffectsBonus then
        return ActorManager5E.getAbilityEffectsBonus(nodeActor, sAbility)
    end
    return 0, 0
end
...
function getAdvDisadvForPerception(nodeCreature)
    ...
    if hasEffectSafe(nodeCreature, "ADVSKILL") then
        bADV = true
    elseif #(getEffectsByTypeSafe(nodeCreature, "ADVSKILL", A_SKILL_FILTER)) > 0 then
        bADV = true
    elseif hasEffectSafe(nodeCreature, "ADVCHK") then
        bADV = true
    elseif #(getEffectsByTypeSafe(nodeCreature, "ADVCHK", A_CHECK_FILTER)) > 0 then
        bADV = true
    end
    if hasEffectSafe(nodeCreature, "DISSKILL") then
        bDIS = true
    elseif #(getEffectsByTypeSafe(nodeCreature, "DISSKILL", A_SKILL_FILTER)) > 0 then
        bDIS = true
    elseif hasEffectSafe(nodeCreature, "DISCHK") then
        bDIS = true
    elseif #(getEffectsByTypeSafe(nodeCreature, "DISCHK", A_CHECK_FILTER)) > 0 then
        bDIS = true
    end
    ...
    local aAddDice, nAddMod, _ = getEffectsBonusSafe(nodeCreature, {"CHECK"}, false, A_CHECK_FILTER)
    local aSkillAddDice, nSkillAddMod, nSkillEffectCount = getEffectsBonusSafe(nodeCreature, {"SKILL"}, false, A_SKILL_FILTER)
    ...
    -- Get ability modifiers
    local nBonusStat, nBonusEffects = getAbilityEffectsBonusSafe(nodeCreature, "wisdom")
...
function getProficiencyBonus(vActor)
    local sNodeType, nodeActor = getTypeAndNodeSafe(vActor);
    local nStatScore
...
function getProficiencyBonusForNPCChallengeRating(nodeActor)
    ...
    if     sCR == "13"
        or sCR == "14"
        or sCR == "15" or sCR == "16" then return 5 end
    ...
    if     sCR == "25"
        or sCR == "26"
        or sCR == "27" or sCR == "28" then return 8 end
...
function getUnidentifiedName(nodeRecord)
    ...
    if isBlankSafe(unidentifiedName) then
